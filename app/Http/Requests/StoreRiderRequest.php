...
class StoreRiderRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            ...
            // Location Messages
            'location.county_id.required' => 'County selection is required.',
            'location.county_id.exists' => 'The selected county does not exist.',
            
            'location.sub_county_id.required' => 'Sub county selection is required.',
            'location.sub_county_id.exists' => 'The selected sub county does not exist.',
            
            'location.ward_id.required' => 'Ward selection is required.',
            'location.ward_id.exists' => 'The selected ward does not exist.',
            
            'location.stage_name.required' => 'Stage/area name is required.',
            'location.stage_name.max' => 'Stage/area name cannot exceed 255 characters.',
            ...
            'location.effective_from.required' => 'Effective from date is required.',
            'location.effective_from.date' => 'Please enter a valid date.',
            'location.effective_from.after_or_equal' => 'Effective from date must be today or a future date.',
            
            'location.notes.max' => 'Location notes cannot exceed 1000 characters.',
    ...
    public function attributes(): array
    {
        return [
            ...
            'location.county_id' => 'county',
            'location.sub_county_id' => 'sub county',
            'location.ward_id' => 'ward',
            'location.stage_name' => 'stage/area name',
            'location.latitude' => 'latitude',
            'location.longitude' => 'longitude',
            'location.effective_from' => 'effective from date',
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('location') && is_array($this->location)) {
            ...
            if (isset($location['latitude']) && $location['latitude'] === '') {
                $location['latitude'] = null;
            }
            if (isset($location['longitude']) && $location['longitude'] === '') {
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($this->filled(['location.county_id', 'location.sub_county_id'])) {
                $subCounty = \App\Models\SubCounty::find($this->input('location.sub_county_id'));
                if ($subCounty && $subCounty->county_id != $this->input('location.county_id')) {
                    $validator->errors()->add('location.sub_county_id', 'The selected sub county does not belong to the selected county.');
                }
            }
            
            if ($this->filled(['location.sub_county_id', 'location.ward_id'])) {
                $ward = \App\Models\Ward::find($this->input('location.ward_id'));
                if ($ward && $ward->sub_county_id != $this->input('location.sub_county_id')) {
                    $validator->errors()->add('location.ward_id', 'The selected ward does not belong to the selected sub county.');
                }
            }
        });
    }
